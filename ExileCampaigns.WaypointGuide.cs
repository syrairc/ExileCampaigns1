...
public partial class ExileCampaigns
{
    ...
    // manually verified slot -> area name, by act. wins over the positional guess where present.
    // acts not listed here still fall back to ActAreaNames while being authored by eye.
    private static readonly Dictionary<int, string[]> WaypointNamesByAct = new()
    {
        [1] = new[]
        {
            "Lioneye's Watch",        // 0
    ...
    private void DrawWaypointNodeIds()
    {
        ...
        var mapped = WaypointNamesByAct.TryGetValue(act, out var m) ? m : null;
